...
void enable(bool enable) {
    if (enable) {
        BST(RCC->APB2ENR, RCC_APB2ENR_IOPAEN);
        // Configure PA12 as an output and pull it down briefly
        // to override the hard-wired external pullup.
        // This must be done before enabling USB because once enabled,
        // USB overrides GPIO control of PA12
        BMD(GPIOA->CRH, (0xFU << 16), (0x1U << 16));
        BCL(GPIOA->ODR, (1 << 12));
        int i;
        for (i = 0; i < 800000; i++) {
            __asm__("nop");
